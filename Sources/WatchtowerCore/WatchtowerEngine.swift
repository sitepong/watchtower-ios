...
final class WatchtowerEngine {
    ...
    func start(apiKey: String, projectId: String, endpoint: URL, sampleRate: Double,
        ...
        observeBackground()
        isRunning = true
        // Structural (view-tree) capture only yields a useful replay for React
        // Native: RN renders into inspectable native views, so the mutation
        // stream reconstructs the real UI. Native SwiftUI/UIKit apps collapse to
        // opaque host views (e.g. _UIHostingView) and produce a blank, useless
        // stream — several thousand no-content mutations per session. So skip it
        // off-RN and rely on the screenshot template player instead. The adapter
        // is internally guarded, so a repeat start is a no-op.
        if platform.hasPrefix("react-native") {
